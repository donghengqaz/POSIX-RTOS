...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  {
        int EthReg = (ETH_ReadPHYRegister(PHYAddress, 0x01));
  }
  ...
  /*------------------------ ETHERNET Configuration --------------------*/
   ...
   //printf("\n\rETH_Read Register %d:0x%X", 0x19, ETH_ReadPHYRegister(PHYAddress, 0x19));
...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  ETH_InitStruct->ETH_Speed = ETH_Speed_100M; //ETH_Speed_10M                      
...
/**
  ...
  * @retval None
  */
void ETH_DMARxDescAddrChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint32_t *RxBuffAddr, uint32_t RxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADESCTypeDef *DMARxDesc;
  ...
  for(i=0; i < RxBuffCount; i++)
  {
    ...
    DMARxDesc->ControlBufferSize = ETH_DMARxDesc_RCH | (uint32_t)ETH_MAX_PACKET_SIZE;  
    /* Set Buffer1 address pointer */
    DMARxDesc->Buffer1Addr = RxBuffAddr[i];
